...
public class AuthRoutes {
    ...
    private Object login(Request req, Response res) {
        ...
        User user = ds.getUsers().stream()
                .filter(u -> u.canLogin())
                .filter(u -> u.getName().equalsIgnoreCase(credential)
                        || credential.equalsIgnoreCase(u.getEmail()))
    ...
    private Object registerUser(Request req, Response res) {
        ...
        if (existingByEmail instanceof GuestClient) {
            ...
            ds.getUsers().replaceAll(u -> u.getId() == upgraded.getId() ? upgraded : u);
            ds.saveUsers();
        ...
        ds.getUsers().add(new Client(ds.nextUserId(), name, email, PasswordUtil.hash(password)));
        ds.saveUsers();
        return JsonUtil.toJson(ApiResponse.ok());
    }
}