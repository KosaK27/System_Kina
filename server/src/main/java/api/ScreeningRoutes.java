...
public class ScreeningRoutes {
    ...
    private Object getByMovie(Request req, Response res) {
        ...
        List<Screening> result = DataStore.getInstance().getScreenings().stream().filter(s -> s.getMovieId() == movieId).toList();
    ...
    private Object create(Request req, Response res) {
        ...
        DataStore.getInstance().insertScreening(s);
    ...
    private Object update(Request req, Response res) {
        ...
        DataStore.getInstance().updateScreening(updated);
    ...
    private Object deleteScreening(Request req, Response res) {
        ...
        AuthFilter.requireRole(req, res, "ADMIN", "EMPLOYEE");
        int id = Integer.parseInt(req.params("id"));
        DataStore.getInstance().deleteScreening(id);
    ...
    private String validate(Screening s, int excludeId) {
        ...
        int duration = ds.getMovies().stream().filter(m -> m.getId() == s.getMovieId()).findFirst().map(model.Movie::getDuration).orElse(120);
        LocalDateTime end = s.getStartTime().plusMinutes(duration + 15);

        boolean collision = ds.getScreenings().stream().filter(x -> x.getId() != excludeId).filter(x -> x.getHallId() == s.getHallId()).anyMatch(x -> {
            int dur = ds.getMovies().stream().filter(m -> m.getId() == x.getMovieId()).findFirst().map(model.Movie::getDuration).orElse(120);
            LocalDateTime xEnd = x.getStartTime().plusMinutes(dur + 15);
            return s.getStartTime().isBefore(xEnd) && end.isAfter(x.getStartTime());
        });
